...
class ChatAdapter(val listener: (ChatItem)->Unit) : RecyclerView.Adapter<ChatAdapter.ChatItemViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChatItemViewHolder {
        ...
        return when(viewType){
            ...
            else -> SingleViewHolder(inflater.inflate(R.layout.item_chat_single,parent,false)) //item_chat_single
        }
...
//        return SingleViewHolder(convertView)
    ...
    fun updateData(data: List<ChatItem>){
        ...
        Log.d("M_ChatAdapter", "updata data adapter - new data ${data.size} hash : ${data.hashCode()}" +
...
//        val tv_title = convertView.findViewById<TextView>(R.id.tv_title_single)
        ...
        override fun bind(item: ChatItem, listener: (ChatItem) -> Unit){
            ...
            with(tv_date_single){
                visibility = if (item.lastMessageDate != null) View.VISIBLE else View.GONE //отладка
                text = item.lastMessageDate
            }
            with(tv_count_single){
